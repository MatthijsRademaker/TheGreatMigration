...
|---|---|---|
| `<BASE_URL>/llms.txt` | Page index with summaries | Swarm or local developer runtime |
...
Read `<BASE_URL>/llms.txt`. This gives you every page title and a one-line summary. Pick the page(s) that match your task.

**Step 2 — Read targeted pages:**
...
**Step 3 — Bulk load only as last resort:**
If you genuinely need everything (rare), read `<BASE_URL>/llms-full.txt`. This is the concatenated full content of every page. It is large. Prefer step 2.
...
```
1. Read llms.txt → discover available docs pages
2. Read the 1-2 .md pages relevant to your task
3. Only then dive into source code — now you know where to look
```

...
### Requirement: REQ-GREET-V2-01 Banner Visibility
...
**Rationale:** The home page at the `/` route is the app's entry point. Placing the banner at the top of the view ensures immediate visibility for anyone validating the swarm pipeline v2 iteration.
...
- **Then** a banner with text "👋 Hello world v2 — swarm pipeline test" is visible as the first child of the `<section>` element in HomeView.vue, above the summary cards grid

### Requirement: REQ-GREET-V2-02 Consistent Styling

**Description:** The banner SHALL use Tailwind CSS v4 utility classes consistent with the project's design system.

**Rationale:** The `bg-muted/40 rounded-lg border p-4` pattern is established in TasksView.vue, PeopleView.vue, CalendarView.vue, and HomeView.vue. The `border-l-4 border-primary` provides a subtle accent consistent with the green/nature-inspired theme. This avoids introducing new shadcn-vue components.
...
### Requirement: REQ-GREET-V2-03 No Side Effects
...
### Requirement: REQ-GREET-V2-04 Build Verification
...
- `src/home/HomeView.vue` — greeting banner

## Notes

- This is a presentational capability only; no interactive behavior
- Banner text includes "v2" to distinguish this pipeline validation iteration from v1
- No feature flag is added — the banner is a pipeline validation marker
...
#### Scenario: Edit prefills the room Select from the task's area id

- **WHEN** a user opens the edit modal for a task
- **THEN** the Room / Area Select is preselected to the task's `area.id`
...
Frontend task rows SHALL use the canonical priority and status vocabulary from the generated client or matching local union types rather than loose `string` status fields. Task rows SHALL consume the `area { id, name }` object from the backend contract (replacing the prior `room` string) and SHALL render the area as a color-coded chip via `areaColor(area.id)`. The displayed `Unassigned` pill SHALL continue to be derived from an empty `assignedTo` array and SHALL NOT become a new canonical backend status value.
...
- **AND** the task's area is consumed as an `{ id, name }` object rather than a `room` string

#### Scenario: Task row renders the area chip

- **WHEN** a task row is rendered
- **THEN** it displays the area name as a chip tinted with `areaColor(area.id)`

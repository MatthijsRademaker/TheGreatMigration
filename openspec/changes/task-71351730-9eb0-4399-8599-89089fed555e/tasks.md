...
- [ ] 1.1 Open `frontend/src/shared/layout/app-sidebar/AppSidebar.vue` and replace all `SidebarMenuButton` nav-item templates: remove `tooltip` prop, remove `as-child` prop, replace `<RouterLink>` with a native `<a>` element that has `:href`, `:title`, and a `@click` handler calling a shared navigation function
- [ ] 1.2 Add a `handleNav(event: MouseEvent, to: string)` function in the script setup of `AppSidebar.vue` that calls `router.push(to)` for standard left-clicks (button === 0 and no modifier keys) or lets the native `href` handle alternative interactions (right-click, middle-click, Ctrl+click)
...
- [ ] 3.2 Verify no `<SidebarMenuButton>` for a navigation item passes `tooltip` or `as-child` props

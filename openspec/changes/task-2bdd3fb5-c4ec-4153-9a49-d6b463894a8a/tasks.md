...
- [ ] 1.1 Create `openspec/changes/task-2bdd3fb5-c4ec-4153-9a49-d6b463894a8a/home-page-ascii.md` with a preamble stating it is a non-canonical structural reference, not a visual spec.
- [ ] 1.2 Add a legend section mapping each dashboard region to its corresponding repo source:
...
- [ ] 1.3 Add a desktop-width monospaced ASCII wireframe (expanded sidebar) with:
...
- [ ] 1.4 Add a collapsed-sidebar ASCII variant showing how the content area reflows when `AppSidebar` is collapsed to icon-only.
- [ ] 1.5 Add a gap-analysis section cataloging deviations between current `HomeView.vue` and the target composition:
...
- [ ] 2.1 Verify the artifact does not modify any file under `openspec/specs/`, `docs/`, `frontend/src/`, or `backend/`.
- [ ] 2.2 Confirm all section labels use existing repo/spec vocabulary (no invented terminology).
- [ ] 2.3 Confirm the legend includes exact file paths and spec references (not directory names).
- [ ] 2.4 Confirm the four KPI cards are enumerated by name and mapped to their API contracts.
- [ ] 2.5 Confirm Daily Schedule and Move Notes are annotated as aspirational/not-yet-built.
...
This specification defines the shared shell header and sidebar navigation chrome contract for the application. It covers the relocation of the primary sidebar toggle into sidebar-owned chrome, the replacement of the static `Planning mode` badge with a read-only planning-window date-range visualizer, the pure formatting utilities that produce display strings from canonical API data, and the SSR test assertions that validate the shell contract. The header is implemented in `AppShell.vue`; the sidebar is implemented in `AppSidebar.vue`; formatting utilities live in `planWindow.ts`; and shell data is provided by `usePlanningWindow.ts`.

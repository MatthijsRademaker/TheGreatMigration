...
This specification defines the frontend-only Task Management panel for the `/tasks` route. It replaces the current placeholder with a static, production-quality Vue composition matching section 4 of `designs/components.png`, while reusing existing shared UI primitives and semantic tokens and deferring backend/data wiring to a later change.

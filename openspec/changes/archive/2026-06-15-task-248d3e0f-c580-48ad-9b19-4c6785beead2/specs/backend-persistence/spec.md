...
Define the Postgres persistence layer for the backend service, establishing pgx connection pooling, sqlc query generation, goose schema migrations, and a `Store` abstraction that decouples handlers from the database.

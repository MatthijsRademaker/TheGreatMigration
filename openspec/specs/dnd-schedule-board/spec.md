# dnd-schedule-board Specification

## Purpose
Drag-and-drop on the schedule board: assign a helper by dragging them from the people rail onto a task card, and reschedule a card by dragging it between day columns. Both persist via `updateScheduleCard` with optimistic UI, reconciliation from the server response, rollback on failure, and drag affordances gated by motion preference. The home page's read-only schedule remains non-interactive.

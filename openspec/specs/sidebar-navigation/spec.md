...
The `RoomsView.vue` and `SettingsView.vue` components SHALL render their content within the existing shared `AppShell` layout. They SHALL NOT introduce their own shell chrome, sidebar, or header. Each view SHALL contain only a minimal placeholder heading and body text. No feature implementation, data fetching, or interactive controls SHALL be present.
...
#### Scenario: SettingsView renders through AppShell with minimal content
...
### Requirement: Shell render tests SHALL assert the expanded sidebar contract

The route-render tests in `frontend/tests/app-routes-render.test.ts` SHALL assert the presence of the six navigation labels (Dashboard, Tasks, Schedule, People, Rooms / Areas, Settings) in the sidebar chrome. Additional route-render cases SHALL cover `/rooms` and `/settings`, asserting their route metadata and content strings. The project card content ("The Great Migration", "House move planner") and utility action labels ("Add note", "Help & Support") SHALL be asserted in the sidebar chrome test.
...
- **AND** a test case exists for `/settings` asserting its route metadata title, description, and placeholder content

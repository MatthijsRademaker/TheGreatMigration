# migration-journey Specification

## Purpose
The signature "Move-Day Readiness Journey": an on-theme progress path (old place → in transit → new place) whose position is derived purely from existing query data (staffing completeness and tool coverage), with no new persisted state. It animates as the underlying data changes and shows an arrived/complete celebration at 100% readiness, gated by motion preference.

...
The backend SHALL register `POST /api/people`, `PUT /api/people/{id}`, and `DELETE /api/people/{id}` through Huma and include them in `/openapi.json`. This first write slice SHALL use client-supplied stable person IDs suitable for name-derived slugs. Successful create and update operations SHALL make the person visible through `GET /api/dashboard/people-availability`. Deleting a person SHALL succeed only when backlog and schedule assignment references do not block the delete.
...
Backend tests SHALL cover successful create/update/delete flows, single-date availability upsert/delete, `400` validation failures, `404` missing people, `409` blocked deletes, OpenAPI path inclusion, and preservation of the existing hello, planning-window, and dashboard-people-availability contracts.
